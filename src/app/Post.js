...
export default function Post() {
  ...
  return (
    <div className="my-14">
      <div className="md:flex gap-6 -m-19">
        {posts.map((post, index) => (
          <div key={index} className="w-90 h-80 mb-6 md:mb-0 mt-40 md:mt-12">
            <Image
              src={post}
              alt={`Post ${index + 1}`}
              width={400}
        ...
        <div className="flex gap-4  w-2.1/3 md:max-h-80">
          ...
          <div>
            ...
            <Image
              ...
              width={330}
              height={130}
              className="md:mt-1 h-40"
            />
          </div>
        </div>
        <div className="w-1.1/3 bg-gray-600 max-h-80">
          <Image src={collagePic} width={700} className="h-80" />
        </div>
      </div>
    </div>
  );
